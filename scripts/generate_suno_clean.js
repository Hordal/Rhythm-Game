#!/usr/bin/env node
// Clean Suno-like generator template (safe copy)
// Usage: node scripts/generate_suno_clean.js --prompt "happy EDM" --bpm 140 --duration 20 --out ai_song_1.wav
...
(async function main() {
  try {
    ...
    const payload = { prompt, bpm, durationSeconds: duration, format: 'wav', quality: 'standard' };
    ...
    if (res.data.audio_base64) {
      fs.writeFileSync(outPath, Buffer.from(res.data.audio_base64, 'base64'));
    ...
    if (err && err.response) console.error('Status:', err.response.status, 'Data:', err.response.data && JSON.stringify(err.response.data).slice(0, 1000));
    process.exit(1);
  }
})();
